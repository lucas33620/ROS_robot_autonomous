...
#endif
  pub_odom_tf(true),
  open_loop(false), //close loop permet d'envoyer une vitesse alors que open loop est un pourcentage d'amperage de temps connecté à la battery
  baud(115200),
  ...
  max_rpm(0)
{
  ...
  nhLocal.param<std::string>("port", port, "/dev/ttyACM1");
  ...
  nhLocal.param("wheel_circumference", wheel_circumference, 0.75);
  ...
  nhLocal.param("max_amps", max_amps, 20.0);
...
void MainNode::cmdvel_setup()
{
  ...
  if (open_loop)
  {
    ROS_INFO_STREAM("open loop activé");
  ...
  {
    ROS_INFO_STREAM("Close-loop speed active");
  ...
  left_rpmcmd << "^MXRPM 1 " << max_rpm << "\r";
  right_rpmcmd << "^MXRPM 2 " << max_rpm << "\r";
  ...
  controller.write("^KP 2 15\r");
  controller.write("^KI 1 75\r");
  controller.write("^KI 2 80\r");
  controller.write("^KD 1 40\r");
  controller.write("^KD 2 45\r");

  // set encoder mode (18 for feedback on motor1, 34 for feedback on motor2)
  controller.write("^EMOD 2 18\r");
  controller.write("^EMOD 1 34\r");

  // set encoder counts (ppr)
  std::stringstream right_enccmd;
  std::stringstream left_enccmd;
  right_enccmd << "^EPPR 1 " << -900 << "\r";
...
*/
  ...
  // determine deltas of distance and angle with reductor ratio
  float linear = 0.031 * ((float)odom_encoder_right / (float)encoder_cpr * wheel_circumference + (float)odom_encoder_left / (float)encoder_cpr * wheel_circumference) / 2.0;
//  float angular = ((float)odom_encoder_right / (float)encoder_cpr * wheel_circumference - (float)odom_encoder_left / (float)encoder_cpr * wheel_circumference) / track_width * -1.0;
  float angular = 0.031 * ((float)odom_encoder_right / (float)encoder_cpr * wheel_circumference - (float)odom_encoder_left / (float)encoder_cpr * wheel_circumference) / track_width;
#ifdef _ODOM_DEBUG
/*
ROS_DEBUG("linear: ");
ROS_DEBUG(linear);
